...
def log_crash_except_hook(exctype, value, tb):
    try:
        with open("crash_log.txt", "a") as f:
            f.write("\n==========================================================\n")
            f.write(f"CRASH DATE/TIME: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("==========================================================\n")
            traceback.print_exception(exctype, value, tb, file=f)
    except Exception as e:
        print(f"[ERROR] Failed to write to crash_log.txt: {e}")
    sys.__excepthook__(exctype, value, tb)

sys.excepthook = log_crash_except_hook

# Optimize Windows scheduler timer resolution for precise time.sleep (important for stable 30 FPS)
...
class VisionConfig:
    ...
    model_path:    str   = os.path.join(os.path.dirname(os.path.abspath(__file__)), "best.pt")      # PyTorch: most stable on GTX 1650
    model_path_pt: str   = os.path.join(os.path.dirname(os.path.abspath(__file__)), "best.pt")      # Fallback PyTorch model
    video_path:    str   = os.path.join(os.path.dirname(os.path.abspath(__file__)), "PFE.mp4")
    ...
    use_tcp:       bool  = False      # True = receive frames from Raspberry Pi over TCP
    tcp_host:      str   = '192.168.137.1'  # Laptop IP on the shared networks
    tcp_port:      int   = 9999
    ...
    conf_thresh:   float = 0.65  # NEW: Low global threshold to capture dark/poorly-lit tissu
    process_every: int   = 4
    display_w:     int   = 1280 
    display_h:     int   = 720
    imgsz:         int   = 640        # YOLO inference size (640 = best accuracy/speed balance for this model)
    use_half:      bool  = False      # FP16 half-precision (set True only on RTX cards with Tensor Cores)

    # User's V4 ROIs (raw 4K) 
    roi_machine: tuple = ((525, 495), (2256, 471), (2460, 726), (426, 846), (528, 498))
    roi_ok:      tuple = ((2541, 1605), (2961, 1503), (3312, 1719), (2877, 1848), (2544, 1605))
    roi_scrap:   tuple = ((2979, 1899), (3402, 1767), (3783, 1992), (3378, 2154), (2985, 1902))
...
# Auto-fallback: if optimized model doesn't exist, use PyTorch model
if not os.path.exists(cfg.model_path):
    print(f"[WARN] Model not found: {cfg.model_path}, falling back to PyTorch: {cfg.model_path_pt}")
    cfg.model_path = cfg.model_path_pt

if cfg.model_path.endswith('.engine'):
    model = YOLO(cfg.model_path, task='detect')
    print(f"[INFO] Loaded TensorRT engine (GPU-optimized for this hardware)")
elif cfg.model_path.endswith('.onnx'):
    model = YOLO(cfg.model_path, task='detect')
    print(f"[INFO] Loaded ONNX model (GPU-accelerated via onnxruntime)")
else:
    model = YOLO(cfg.model_path).to(device)
    print(f"[INFO] Loaded PyTorch model on {device.upper()}")

# FP16 half-precision: handled by passing half=True to model() calls
if cfg.use_half and device == "cuda":
    print("[PERF] FP16 half-precision ENABLED (will apply during inference)")

# CUDA Warmup: first inference is always slow due to kernel/torch.compile compilation.
# Run 5 dummy frames to pre-warm the GPU pipeline before real processing.
if device == "cuda":
    print("[PERF] Warming up CUDA kernels (5 dummy inferences)...", end=" ", flush=True)
...
while cap.isOpened():
    if not paused:
        ...
        # Throttle video file playback to real-time (VIDEO_FPS) using drift-compensated scheduling
        if not (cfg.use_tcp or cfg.use_camera):
            frame_delay = 1.0 / VIDEO_FPS
            target_time = loop_start_time + frame_count * frame_delay
            now = time.time()
            sleep_time = target_time - now
            if sleep_time > 0:
                time.sleep(sleep_time)
            elif sleep_time < -0.1:
                # Shift loop_start_time forward on resume or severe lag to realign target
                loop_start_time = time.time() - frame_count * frame_delay
        ...
        if now - fps_timer >= 1.0:
            # Calculate actual FPS based on frames processed
        ...
        if frame_count % cfg.process_every == 0:
            ...
            # Print latency breakdown to identify bottlenecks
            if frame_count % 10 == 0:
                t_end = time.time()
                yolo_ms = (t_after_yolo - t_frame_start) * 1000
                post_ms = (t_end - t_after_yolo) * 1000
                total_ms = (t_end - t_frame_start) * 1000
                print(f"[LATENCY] YOLO: {yolo_ms:.1f}ms | Post+MQTT: {post_ms:.1f}ms | Total: {total_ms:.1f}ms")
